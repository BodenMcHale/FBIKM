...
const FBIKM_NODE_ID = 3  # THIS NODE'S IDENTIFIER

enum SIDE {UP, DOWN, LEFT, RIGHT, FORWARD, BACK}

var bone_id: String = "-1"
@export var look_from_side: SIDE = SIDE.UP ## A side of the bone which will turn towards the target
...
func _connect_to_parent() -> void:
	...
	if parent and parent.has_method("get") and parent.get("FBIKM_NODE_ID") == 0:
		print("IKLookAt connecting to parent manager")
		if not parent.bone_names_obtained.is_connected(_update_parameters):
			parent.bone_names_obtained.connect(_update_parameters)
			print("IKLookAt connected successfully")
		else:
			print("IKLookAt already connected")
	else:
		print("IKLookAt: Parent is not a KinematicsManager")
...
func _update_parameters(bone_names: String) -> void:
	print("IKLookAt received bone names: ", bone_names)
